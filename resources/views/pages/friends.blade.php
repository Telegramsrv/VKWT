    ...
    @foreach ( $FriendStats as $Friend)
        ...
        <div class="row bg-white" style="height:auto">
            ...
            <div class="col-sm-3">
                ...
                <a style="color: gray" href="http://vk.com/id{{$Friend[1]['user_id']}}" >VK</a></h2>
            </div>
            @if(count($Friend[0])>1)
                <div id="curve_chart{{ $Friend[1]['user_id'] }}" style="width: 90%; height: 300px" class="col-md-5"></div>
            @else
                <h2 style="color: orangered">У данного пользователя недостаточно записей на стине для статистики!</h2>
            @endif
        </div><br/>
    @endforeach
